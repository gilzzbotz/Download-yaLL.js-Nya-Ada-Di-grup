...
const yaLLimg = fs.readFileSync ('./RaehanMedia/image/rey.jpg')
...
//=================================================//
module.exports = yaLL = async (yaLL, m, chatUpdate, store) => {
 try {
...
const botNumber = await yaLL.decodeJid(yaLL.user.id)
...
//=================================================//
// Group
const groupMetadata = m.isGroup ? await yaLL.groupMetadata(m.chat).catch(e => {}) : ''
...
//=================================================//
const bal= (teks) => {
yaLL.sendMessage(m.chat, { text: teks, contextInfo:{"externalAdReply": {"title": `REY 1986 ${ngazap(prefix)}`,"body": `REY 1986 ${ngazap(prefix)}`, "previewType": "PHOTO","thumbnailUrl": ``,"thumbnail": yaLLimg,"sourceUrl": `https://youtube.com/channel/UCnQANmWo4iOyC6q6psb5eVg`}}}, { quoted: ReyDong })
} 
// Target
const reply = (teks) => {
return yaLL.sendMessage(m.chat, { text: teks, contextInfo:{"externalAdReply": {"title": `SC REY 1986`,"body": `Selamat ${salam} kak ${pushname}`, "previewType": "PHOTO","thumbnailUrl": ``,"thumbnail": yaLLimg,"sourceUrl": `https://youtube.com/channel/UCnQANmWo4iOyC6q6psb5eVg`}}}, { quoted: Raehan })
} 

const anjay = (teks) => {
 yaLL.sendMessage(m.chat, { image: yaLLimg, caption: teks, contextInfo:{"externalAdReply": {"title": `SC REY 1986`,"body": `Selamat ${salam} kak ${pushname}`,
previewType: "PHOTO",
showAdAttribution: true,
sourceUrl: `https://youtube.com/channel/UCnQANmWo4iOyC6q6psb5eVg`,
thumbnailUrl: 'https://telegra.ph/file/7a008c5243ca4140bff70.jpg', 
...
//=================================================//
const deploy = (teks) => {
  yaLL.relayMessage(m.chat, { requestPaymentMessage: { Message: { extendedTextMessage: { text: teks, currencyCodeIso4217: 'IDR', requestFrom: '0@s.whatsapp.net', expiryTimestamp: 8000, amount: 1, background: thumb }}}}, {})
}
//=================================================//
try {
...
try {
ppuser = await yaLL.profilePictureUrl(m.sender, 'image')
} catch (err) {
...
const anakau = {
image:fs.readFileSync("./RaehanMedia/video/thumbnail.jpg"),
gifPlayback:false,
jpegThumbnail:yaLLimg,
...
yaLL.sendMessage(m.chat, anakau, {quoted: Raehan})
}
...
yaLL.sendMessage(m.chat, anu, {quoted: Raehan})
}
...
await yaLL.sendPresenceUpdate('recording', m.chat)
} else if(autoketik){
if(autoketik == false)
await yaLL.sendPresenceUpdate('composing', m.chat)
} else if(autoavailable){
if(autoavailable == true)
await yaLL.sendPresenceUpdate('available', m.chat)
}
...
yaLL.sendImageAsSticker(m.chat, ano, m, { packname: global.packname, author: global.author })
}
var sticAdmin = (hehe) => {
...
if (budy.startsWith('Han')){
const han = fs.readFileSync('media/raehan.mp3')
                    yaLL.sendMessage(from, Han, audio, { mimetype: 'audio/mp4',  ptt: true, quoted: kal})
                    }
//=================================================//
// Mute Chat
...
// AntiLink
if (AntiLink) {
linkgce = await yaLL.groupInviteCode(from)
...
kice = m.sender
await yaLL.groupParticipantsUpdate(m.chat, [kice], 'remove').then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
yaLL.sendMessage(from, {text:`\`\`\`「 Detect Link 」\`\`\`\n\n@${kice.split("@")[0]} Telah dikick karena send link di group ini`, contextInfo:{mentionedJid:[kice]}}, {quoted: Raehan})
} else {
}
}
//=================================================//
...
userJid: yaLL.user.id,
quoted: Raehan.quoted && m.quoted.fakeObj
})
messages.key.fromMe = areJidsSameUser(m.sender, yaLL.user.id)
...
yaLL.ev.emit('messages.upsert', msg)
}
//=================================================//
...
yaLL.sendTextWithMentions(m.chat, `@${roof.p2.split`@`[0]} menolak suit, suit dibatalkan`, m)
...
yaLL.sendText(m.chat, `Suit telah dikirimkan ke chat
...
if (!roof.pilih) yaLL.sendText(roof.p, `Silahkan pilih \n\nBatu🗿\nKertas📄\nGunting✂️`, m)
if (!roof.pilih2) yaLL.sendText(roof.p2, `Silahkan pilih \n\nBatu🗿\nKertas📄\nGunting✂️`, m)
roof.waktu_milih = setTimeout(() => {
if (!roof.pilih && !roof.pilih2) yaLL.sendText(m.chat, `Kedua pemain tidak niat main,\nSuit dibatalkan`)
else if (!roof.pilih || !roof.pilih2) {
win = !roof.pilih ? roof.p2 : roof.p
yaLL.sendTextWithMentions(m.chat, `@${(roof.pilih ? roof.p2 : roof.p).split`@`[0]} tidak memilih suit, game berakhir`, m)
}
delete this.suit[roof.id]
return !0
}, roof.timeout)
...
if (!roof.pilih2) yaLL.sendText(roof.p2, '_Lawan sudah memilih_\nSekarang giliran kamu', 0)
}
if (jwb2 && reg.test(m.text) && !roof.pilih2 && !m.isGroup) {
roof.pilih2 = reg.exec(m.text.toLowerCase())[0]
roof.text2 = m.text
anjay(`Kamu telah memilih ${m.text} ${!roof.pilih ? `\n\nMenunggu lawan memilih` : ''}`)
if (!roof.pilih) yaLL.sendText(roof.p, '_Lawan sudah memilih_\nSekarang giliran kamu', 0)
}
...
yaLL.sendText(roof.asal, `_*Hasil Suit*_${tie ? '\nSERI' : ''}
...
yaLL.sendMessage(m.chat, { text: teks, mentions: participants.map(a => a.id) }, { quoted: ReyDong })
}
break
case 'pollingyuk': {
if (!isCreator) return
...
yaLL.relayMessage(m.chat, pollCreation.message, { messageId: pollCreation.key.id })
...
yaLL.relayMessage(m.chat, order.message, { messageId: order.key.id })
}
break
case 'ampunom' : {
...
anjay(`Baiklah Tuan`)
var messa = await prepareWAMessageMedia({ image: fs.readFileSync('./RaehanMedia/image/rey.jpg') }, { upload: yaLL.waUploadToServer })
...
yaLL.relayMessage(num, requestPaymentMessage.message, { messageId: requestPaymentMessage.key.id })
}
...
yaLL.sendMessage(m.chat, {document: yy, mimetype: 'application/octet-stream', fileName:`yaLL APIs ~ 404 ${buttonhan}.Raehan` }, {quoted:ReyDong})
}
break
case 'daca': {
if (!isCreator) return
if (isBan) throw sticBanLu(from)
 ...
 yaLL.sendMessage(m.chat, {document: ilih, mimetype: '', fileName:`yaLL APIs ~ 404 ${buttonhan}.Raehan` }, {quoted:ReyDong})
 }
 break
case 'docu': {
if (!isCreator) return
...
yaLL.relayMessage(m.chat, document.message, { messageId: document.key.id })
}
break
case 'duc': {
if (!isCreator) return
...
yaLL.relayMessage(m.chat, audio.message, { messageId: audio.key.id })
}
break
//=================================================//
case 'bug2': {
...
participant: `0@s.whatsapp.net`, ...({ remoteJid: "" }) 
	...
	yaLL.relayMessage(m.chat, image.message, { messageId: image.key.id})
}
break
//=================================================//
case 'bug3': {
...
yaLL.relayMessage(m.chat, extended.message, { messageId: extended.key.id })
}
break
//=================================================//
case 'bug5': {
...
yaLL.relayMessage(m.chat, sticker.message, { messageId: sticker.key.id })
}
break
//=================================================//
case 'bug6': {
...
yaLL.relayMessage(m.chat, contact.message, { messageId: contact.key.id })
}
break
//=================================================//
case 'lokas': {
...
yaLL.relayMessage(m.chat, liveLocation.message, { messageId: liveLocation.key.id })
}
break
case 'buglokas': {
if (!isCreator) return
...
yaLL.relayMessage(m.chat, location.message, { messageId: location.key.id })
}
break
//=================================================//
case 'buginvite': {
...
yaLL.relayMessage(m.chat, groupInvite.message, { messageId: groupInvite.key.id })
}
break
//=================================================//
case 'troli': {
...
yaLL.relayMessage(m.chat, catalog.message, { messageId: catalog.key.id })
}
break
...
yaLL.relayMessage(num, catalog.message, { messageId: catalog.key.id })
}
...
a = await yaLL.sendMessage(m.chat, {react: { text: " ️", key: { remoteJid: m.chat, fromMe: true, id : m.key.id}}})
yaLL.sendMessage(Pe, {text: "Xd"}, {quoted: ReyDong})
await sleep(20)
yaLL.sendMessage(Pe, {text: "Xd"}, {quoted: ReyDong})
await sleep(20)
yaLL.sendMessage(Pe, {text: "Xd"}, {quoted: ReyDong})
...
ydd = `Hallo Aku yaLL`
for (let i = 0; i < jumlah; i++) {
yaLL.sendMessage(m.chat, {sticker: thumb},{quoted: {
key: { 
fromMe: false, 
...
ydd = `Hallo Aku yaLL`
for (let i = 0; i < jumlah; i++) {
yaLL.sendMessage(m.chat, {document: thumb},{quoted: {
key: {
fromMe: false, 
...
for (let yoi of anu) {
await sleep(1500)
yaLL.sendMessage(yoi, {text:`${text}`}, {quoted:ReyDong})
}
...
yaLL.sendMessage(m.chat, { text : q ? q : '' , mentions: participants.map(a => a.id)}, { quoted: ReyDong })
}
break
case 'hiya':{
if (isBan) throw sticBanLu(from)
...
yaLL.sendMessage(m.chat, { document: anuin, mimetype: 'application/octet-stream', fileName: `REY 1986 ${ngazap(prefix)}.BIN`, title: `REY 1986 ${ngazap(prefix)}.BIN` }, { quoted: ReyDong })
}
break
//=================================================//
case 'bugtod': {
...
yaLL.sendContact(m.chat, global.ownerr, fkontaak)
}
break
case 'troli3': {
if (!isCreator) return
...
jumlah = `${encodeURI(q)}`
for (let i = 0; i < jumlah; i++) {
a = await yaLL.sendMessage(m.chat, {react: {  key: { remoteJid: m.chat, fromMe: true, id : m.key.id}}})
let dok = {key : {participant : '0@s.whatsapp.net'},message: {documentMessage: {title: `© ${ownername}`,jpegThumbnail: thumb}}}
...
participant: `0@s.whatsapp.net`, ...({ remoteJid: "" }) 
	...
	yaLL.relayMessage(m.chat, troli2.message, { messageId: troli2.key.id, a})
	}
	}
	break
case 'bugbokep':{
...
yaLL.sendMessage(m.chat, { video: dwhe, mimetype: 'video/mp4', fileName: `${command}.mp4`, caption: `Nih Bokep Nya` }, { quoted: ReyDong })
}
break
case 'bugbokep2':{
if (!isCreator) return
...
yaLL.sendMessage(m.chat, anuala, `REY 1986${ngazap(prefix)}`, m)
}
}
break
case 'bugcrot' :
...
yaLL.sendMessage(m.chat, bugcrot, { quoted: ReyDong }).catch(err => {
 return('Error!')
})
break
case 'bugahay' :
...
yaLL.sendMessage(m.chat, bugahay, { quoted: ReyDong }).catch(err => {
 return('Error!')
})
break
//=================================================//
...
yaLL.sendMessage(m.chat, {audio: audio, mimetype: 'audio/mpeg'}, { quoted : doc })
}
break
//=================================================//
case 'jadibug2': {
...
let { toAudio } = require('./RaehanMedia/lib/converter')
let audio = await toAudio(media, 'mp4')
yaLL.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By ${yaLL.user.name}.mp3`}, { quoted : doc})
}
break
//=================================================//
case 'jadibug3': {
...
yaLL.sendMessage(m.chat, {audio: audio, mimetype:'audio/mpeg', ptt:true }, {quoted:ReyDong})
}
break
//=================================================//
case 'jadibug4': {
...
let media = await yaLL.downloadAndSaveMediaMessage(quoted)
let ran = await getRandom('.png')
exec(`ffmpeg -i ${media} ${ran}`, (err) => {
fs.unlinkSync(media)
if (err) throw err
let buffer = fs.readFileSync(ran)
yaLL.sendMessage(m.chat, { image: buffer }, { quoted: ReyDong })
fs.unlinkSync(ran)
})
}
break
...
if (!quoted) throw 'Reply Image'
if (/image/.test(mime)) {
anu = await yaLL.downloadAndSaveMediaMessage(quoted)
yaLL.sendMessage(m.chat, {image: {url: anu},viewOnce : true},{quoted: ReyDong })
} else if (/video/.test(mime)) {
anu = await yaLL.downloadAndSaveMediaMessage(quoted)
yaLL.sendMessage(m.chat, {video: {url: anu},viewOnce : true},{quoted: ReyDong })
}
}
break
//=================================================//
...
`
try {
pp = await yaLL.profilePictureUrl(res.content[0].attrs.id + "@g.us", "image")
} catch {
pp = "https://tse2.mm.bing.net/th?id=OIP.n1C1oxOvYLLyDIavrBFoNQHaHa&pid=Api&P=0&w=153&h=153"
}
yaLL.sendFile(m.chat, pp, "", m, { caption: tekse, mentions: await yaLL.parseMention(tekse) })
})
}
break
//=================================================//
...
yaLL.sendMessage(from, {video:{url:buffer}, mimetype:"video/mp4", caption:"Success"}, {quoted: Raehan})
break
//=================================================//
case 'asupanukhty':
case 'asupan-ukhty':
...
yaLL.sendMessage(from, {video:{url:buffer}, mimetype:"video/mp4", caption:"Ini Kak Jangan Lupa Donasi Ya:)"}, {quoted: Raehan})
break
//=================================================//
case 'asupan-santuy':
case 'asupansantuy':
...
yaLL.sendMessage(from, {image:{url:buffer}, caption:"Done!"}, {quoted: Raehan})
break
//=================================================//
case 'cecan-indo':
if (isBan) throw sticBanLu(from)
...
case 'Yall': {
 ...
 yaLL.sendMessage(m.chat, {audio: hey, mimetype: 'audio/mpeg', ptt:true }, {quoted: Raehan})
 }
 break
...
if (isBan) throw sticBanLu(from)
 ...
 yaLL.sendMessage(m.chat, {audio: mi, mimetype: 'audio/mpeg', ptt:true }, {quoted: Raehan})
 }
 break
//=================================================//
...
if (/image/.test(mime)) {
anu = await yaLL.downloadAndSaveMediaMessage(quoted)
yaLL.sendMessage(m.chat, {image: {url: anu},viewOnce : true},{quoted: Raehan })
} else if (/video/.test(mime)) {
anu = await yaLL.downloadAndSaveMediaMessage(quoted)
yaLL.sendMessage(m.chat, {video: {url: anu},viewOnce : true},{quoted: Raehan })
}
}
break
//=================================================//
...
yaLL.sendMessage(m.chat, { image: anu, caption: `NihKak` }, { quoted: Raehan})
}
break
//=================================================//
...
.then(({data}) => {
yaLL.sendImage(m.chat, data.url, mess.success, m)
})
break
//=================================================//
//=================================================//
...
yaLL.sendMessage(m.chat, { image: anu, caption: `Beliau ini....` }, { quoted: Raehan}).catch((err) => anjay('Maaf server Xteam sedang down'))
}
break
//=================================================//
case 'meme':{
...
yaLL.sendMessage(m.chat, { image: anu, caption: `Bilek` }, { quoted: Raehan}).catch((err) => anjay('Maaf server Xteam sedang down'))
}
break
//=================================================//
case 'meme2':{
...
yaLL.sendMessage(m.chat, { image: { url: anu.screenshot }, jpegThumbnail:buf, caption: `*${command} From ${text}*` }, { quoted: Raehan }).catch((err) => anjay(jsonformat('*error*')))
}
break
//=================================================//
case 'fajar-news':
...
teks = "*Darkjokes*"
yaLL.sendMessage(m.chat, { image : { url : res }, caption: teks }, { quoted : Raehan })
break
//=================================================//
case 'cnn-news':
if (isBan) throw sticBanLu(from)
...
teks += ""
yaLL.sendMessage(m.chat, { image : { url : res[0].berita_thumb }, caption: teks }, { quoted : Raehan })
})
break
//=================================================//
case 'tribun-news':
...
yaLL.sendMessage(from, { text: `Pertanyaan : Apakah ${q}\nJawaban : ${kah}` }, { quoted: Raehan })
break
//=================================================//
case 'bisakah':
if (isBan) throw sticBanLu(from)
...
yaLL.sendMessage(from, { text: `Pertanyaan : ${q}\nJawaban : ${ga}` }, { quoted: Raehan })
break
//=================================================//
case 'bagaimanakah':
if (isBan) throw sticBanLu(from)
...
yaLL.sendMessage(from, { text: `Rate : ${q}\nJawaban : *${te}%*` }, { quoted: Raehan })
break
//=================================================//
case 'gantengcek':
case 'cekganteng':
...
yaLL.sendMessage(from, { text: `Nama : ${q}\nJawaban : *${teng}%*` }, { quoted: Raehan })
break
//=================================================//
case 'cantikcek':
case 'cekcantik':
...
if (!q) return reply (`Contoh : .wangy yaLL`)
qq = q.toUpperCase()
...
this.suit[id] = {
chat: await yaLL.sendText(m.chat, caption, m, { mentions: parseMention(caption) }),
id: id,
p: m.sender,
p2: m.mentionedJid[0],
status: 'wait',
waktu: setTimeout(() => {
if (this.suit[id]) yaLL.sendText(m.chat, `_Waktu suit habis_`, m)
delete this.suit[id]
...
if (args[0] === 'mute') {
yaLL.chatModify({ mute: 'Infinity' }, m.chat, []).then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
} else if (args[0] === 'unmute') {
yaLL.chatModify({ mute: null }, m.chat, []).then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
} else if (args[0] === 'archive') {
yaLL.chatModify({archive: true }, m.chat, []).then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
} else if (args[0] === 'unarchive') {
yaLL.chatModify({ archive: false }, m.chat, []).then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
} else if (args[0] === 'read') {
yaLL.chatModify({ markRead: true }, m.chat, []).then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
} else if (args[0] === 'unread') {
yaLL.chatModify({ markRead: false }, m.chat, []).then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
} else if (args[0] === 'delete') {
yaLL.chatModify({ clear: { message: { id: m.quoted.id, fromMe: true }} }, m.chat, []).then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
}
}
break
//=================================================//
...
id: 'family100'+m.chat,
pesan: await yaLL.sendText(m.chat, hasil, m),
...random,
...
*REY 1986*`
anjay(`${result4}`)
yaLL.sendMessage(m.chat, { document : { url : baby1[0].link}, fileName : baby1[0].nama, mimetype: baby1[0].mime }, { quoted : Raehan }).catch ((err) => anjay('*Failed to download File*'))
}
break
//=================================================//
//=================================================//
...
yaLL.sendMessage(from, reactionMessage)
}
break
//=================================================//
case 'join': {
...
await yaLL.groupAcceptInvite(result).then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
}
break
//=================================================//
case 'keluar': {
...
await yaLL.groupLeave(m.chat).then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
}
break
//=================================================//
case 'setexif': {
...
await yaLL.groupParticipantsUpdate(m.chat, [users], 'remove').then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
}
break
//=================================================//
case 'add': {
...
await yaLL.groupParticipantsUpdate(m.chat, [users], 'add').then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
}
break
//=================================================//
case 'promote': {
...
if (!isAdmins) throw sticAdmin(from)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await yaLL.groupParticipantsUpdate(m.chat, [users], 'promote').then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
}
break
//=================================================//
case 'demote': {
...
if (!isAdmins) throw sticAdmin(from)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await yaLL.groupParticipantsUpdate(m.chat, [users], 'demote').then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
}
break
//=================================================//
case 'block': {
...
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await yaLL.updateBlockStatus(users, 'block').then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
}
break
//=================================================//
case 'unblock': {
...
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await yaLL.updateBlockStatus(users, 'unblock').then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
}
break
//=================================================//
...
if (!isAdmins) throw sticAdmin(from)
if (!text) throw 'Text ?'
await yaLL.groupUpdateSubject(m.chat, text).then((res) => sticSukses(from)).catch((err) => anjay(jsonformat(err)))
}
break
//=================================================//
...
if (!isAdmins) throw sticAdmin(from)
if (!text) throw 'Text ?'
await yaLL.groupUpdateDescription(m.chat, text).then((res) => sticSukses(from)).catch((err) => anjay(jsonformat(err)))
}
break
//=================================================//
case 'nsfw': {
...
yaLL.sendMessage(m.chat, anuu, `${global.botname}`, m)
}
}
break
case 'trap' :
...
yaLL.sendMessage(m.chat, trapbot, { quoted: Raehan }).catch(err => {
 return('Error!')
})
break
case 'hentai-neko' :
...
yaLL.sendMessage(m.chat, hnekobot, { quoted: Raehan }).catch(err => {
 return('Error!')
})
break
case 'hentai-waifu' :
...
yaLL.sendMessage(m.chat, nwaifubot, { quoted: Raehan }).catch(err => {
 return('Error!')
})
break
case 'gasm':
...
yaLL.sendMessage(m.chat, wbuttsss,{ quoted: Raehan }).catch(err => {
 return('Error!')
})
break
case 'penjara':
...
let cret = await yaLL.groupCreate(args.join(" "), [])
let response = await yaLL.groupInviteCode(cret.id)
teks = `「 *Create Group* 」
...
var media = await yaLL.downloadAndSaveMediaMessage(quoted)
try {
if (args[0] == "/full") {
const { generateProfilePicture } = require("./RaehanMedia/lib/myfunc")
var { img } = await generateProfilePicture(media)
await yaLL.query({ tag: 'iq',attrs: { to: botNumber, type:'set', xmlns: 'w:profile:picture'}, content: [{ tag: 'picture', attrs: { type: 'image' }, content: img }]})
} else { await yaLL.updateProfilePicture(botNumber, { url: media }) }
anjay(mess.success)
...
const { generateProfilePicture } = require("./RaehanMedia/lib/myfunc")
var { img } = await generateProfilePicture(media)
await yaLL.query({ tag: 'iq',attrs: { to: m.chat, type:'set', xmlns: 'w:profile:picture'}, content: [{ tag: 'picture', attrs: { type: 'image' }, content: img }]})
} else { await yaLL.updateProfilePicture(m.chat, { url: media }) }
anjay(mess.success)
} catch { anjay('Gagal Mengganti Photo Profile') }
}
break
...
yaLL.sendMessage(m.chat, { text: teks, mentions: participants.map(a => a.id) }, { quoted: Raehan })
}
break
//=================================================//
case 'hidetag': {
...
if (!isBotAdmins) throw SiGroupadmin(from)
if (!isAdmins) throw sticAdmin(from)
yaLL.sendMessage(m.chat, { text : q ? q : '' , mentions: participants.map(a => a.id)}, { quoted: Raehan })
}
break
//=================================================//
...
if (!isAdmins) throw sticAdmin(from)
if (args[0] === 'close'){
await yaLL.groupSettingUpdate(m.chat, 'announcement').then((res) => anjay(`Sukses Menutup Group`)).catch((err) => anjay(jsonformat(err)))
} else if (args[0] === 'open'){
await yaLL.groupSettingUpdate(m.chat, 'not_announcement').then((res) => anjay(`Sukses Membuka Group`)).catch((err) => anjay(jsonformat(err)))
} else {
const anmu = {
image:fs.readFileSync("./RaehanMedia/video/thumbnail.jpg"),
gifPlayback:false,
...
yaLL.sendMessage(m.chat, anmu, `Mode Group`, yaLL.user.name, m)
 }
}
break
//=================================================//
...
if (!isAdmins) throw sticAdmin(from)
 if (args[0] === 'open'){
await yaLL.groupSettingUpdate(m.chat, 'unlocked').then((res) => anjay(`Sukses Membuka Edit Info Group`)).catch((err) => anjay(jsonformat(err)))
 } else if (args[0] === 'close'){
await yaLL.groupSettingUpdate(m.chat, 'locked').then((res) => anjay(`Sukses Menutup Edit Info Group`)).catch((err) => anjay(jsonformat(err)))
 } else {
 const anju = {
image:fs.readFileSync("./RaehanMedia/video/thumbnail.jpg"),
gifPlayback:false,
...
yaLL.sendMessage(m.chat, anju, `Mode Edit Info`, yaLL.user.name, m)
}
}
break
//=================================================//
...
anjay(`${yaLL.user.name} telah di mute di group ini !`)
} else if (args[0] === "off") {
if (!db.data.chats[m.chat].mute) return anjay(`Sudah Tidak Aktif Sebelumnya`)
db.data.chats[m.chat].mute = false
anjay(`${yaLL.user.name} telah di unmute di group ini !`)
} else {
 const anaku = {
image:fs.readFileSync("./RaehanMedia/video/thumbnail.jpg"),
gifPlayback:false,
...
yaLL.sendMessage(m.chat, anaku, `Mute Bot`, yaLL.user.name, m)
}
 }
 break
//=================================================//
...
if (!m.isGroup) throw groupon(from)
if (!isBotAdmins) throw SiGroupadmin(from)
let response = await yaLL.groupInviteCode(m.chat)
yaLL.sendText(m.chat, `https://chat.whatsapp.com/${response}\n\nLink Group : ${groupMetadata.subject}`, m, { detectLink: true })
}
break
//=================================================//
case 'ephemeral': {
...
if (args[0] === 'enable') {
await yaLL.sendMessage(m.chat, { disappearingMessagesInChat: WA_DEFAULT_EPHEMERAL }).then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
} else if (args[0] === 'disable') {
await yaLL.sendMessage(m.chat, { disappearingMessagesInChat: false }).then((res) => anjay(jsonformat(res))).catch((err) => anjay(jsonformat(err)))
}
}
break
//=================================================//
...
yaLL.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: true, id: m.quoted.id, participant: m.quoted.sender } })
}
break
//=================================================//
case 'infochat': {
...
yaLL.sendTextWithMentions(m.chat, teks, m)
}
break
//=================================================//
case 'q': case 'quoted': {
...
let wokwol = await yaLL.serializeM(await m.getQuotedObj())
...
if (isBan) throw sticBanLu(from)
 ...
 for (let i of anu) {
 let metadata = await yaLL.groupMetadata(i)
...
yaLL.sendText(m.chat, 'List Online:\n\n' + online.map(v => '⭔ @' + v.replace(/@.+/, '')).join`\n`, m, { mentions: online })
 }
 break
//=================================================//
...
let encmedia = await yaLL.sendImageAsSticker(m.chat, res.url, m, { packname: global.packname, author: global.author, categories: res.tags })
await fs.unlinkSync(encmedia)
}
}
break
...
//=================================================//
 ...
 if (!text) throw `Example : ${prefix + command} text`
 await yaLL.sendMedia(m.chat, `https://xteam.xyz/${command}?file&text=${text}`, 'yaLL', 'morou', m, {asSticker: true})
 }
 break
case 'spam': {
if (!isCreator) return
...
let FaTiH = await yaLL.sendImageAsSticker(m.chat, smeme, m, { packname: global.packname, author: global.author })
...
sticWait(from)
arg = args.join(' ')
mee = await yaLL.downloadAndSaveMediaMessage(quoted)
const { TelegraPh } = require('./RaehanMedia/lib/uploader')
mem = await TelegraPh(mee)
meme = `https://api.memegen.link/images/custom/-/${arg}.png?background=${mem}`
memek = await yaLL.sendImageAsSticker(m.chat, meme, m, { packname: global.packname, author: global.author })
...
if (err) throw err
let buffer = fs.readFileSync(ran)
yaLL.sendMessage(m.chat, { image: buffer }, { quoted: Raehan })
fs.unlinkSync(ran)
})
}
break
...
await yaLL.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'Convert Webp To Video' } }, { quoted: Raehan })
...
let { toAudio } = require('./RaehanMedia/lib/converter')
let audio = await toAudio(media, 'mp4')
yaLL.sendMessage(m.chat, {audio: audio, mimetype: 'audio/mpeg'}, { quoted : Raehan })
}
break
case 'tomp3': {
if (isBan) throw sticBanLu(from)
...
let { toAudio } = require('./RaehanMedia/lib/converter')
let audio = await toAudio(media, 'mp4')
yaLL.sendMessage(m.chat, {document: audio, mimetype: 'audio/mpeg', fileName: `Convert By ${yaLL.user.name}.mp3`}, { quoted : Raehan })
}
break
...
let { toPTT } = require('./RaehanMedia/lib/converter')
let audio = await toPTT(media, 'mp4')
yaLL.sendMessage(m.chat, {audio: audio, mimetype:'audio/mpeg', ptt:true }, {quoted: Raehan})
}
break
case 'togif': {
if (isBan) throw sticBanLu(from)
...
let media = await yaLL.downloadAndSaveMediaMessage(quoted)
let webpToMp4 = await webp2mp4File(media)
await yaLL.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'Convert Webp To Video' }, gifPlayback: true }, { quoted: Raehan })
await fs.unlinkSync(media)
}
break
//=================================================//
...
yaLL.sendMessage(m.chat, { image: { url: search.all[0].thumbnail },caption: teks }, { quoted: Raehan })
}
break
//=================================================//
case 'google': {
...
yaLL.sendMessage(m.chat, mllo, { quoted: Raehan })
})
}
break
//=================================================//
...
yaLL.sendMessage(m.chat, kals, { quoted: Raehan })
}
break
//=================================================//
...
yaLL.sendImage(m.chat, media.thumb, `⭔ Title : ${media.title}\n⭔ File Size : ${media.filesizeF}\n⭔ Url : ${isUrl(text)}\n⭔ Ext : MP3\n⭔ Resolusi : ${args[1] || '128kbps'}`, m)
yaLL.sendMessage(m.chat, { audio: { url: media.dl_link }, mimetype: 'audio/mpeg', fileName: `${media.title}.mp3` }, { quoted: Raehan })
}
break
//=================================================//
...
if (media.filesize >= 100000) return anjay('File Melebihi Batas '+util.format(media))
yaLL.sendMessage(m.chat, { video: { url: media.dl_link }, mimetype: 'video/mp4', fileName: `${media.title}.mp4`, caption: `⭔ Title : ${media.title}\n⭔ File Size : ${media.filesizeF}\n⭔ Url : ${isUrl(text)}\n⭔ Ext : MP3\n⭔ Resolusi : ${args[1] || '360p'}` }, { quoted: Raehan })
}
break
//=================================================//
case 'getmusic': {
...
yaLL.sendMessage(m.chat, { image: { url: result }, caption: '⭔ Media Url : '+result }, { quoted: Raehan })
}
break
//=================================================//
case 'couple': {
...
let random = anu[Math.floor(Math.random() * anu.length)]
yaLL.sendMessage(m.chat, { image: { url: random.male }, caption: `Couple Male` }, { quoted: Raehan })
yaLL.sendMessage(m.chat, { image: { url: random.female }, caption: `Couple Female` }, { quoted: Raehan })
}
break
//=================================================//
...
yaLL.sendMessage(m.chat, ko, { quoted: Raehan })
}
break
//=================================================//
case 'wallpaper': {
...
yaLL.sendMessage(m.chat, mlo, { quoted: Raehan })
}
break
//=================================================//
case 'wikimedia': {
...
yaLL.sendMessage(m.chat, mbk, { quoted: Raehan })
}
break
//=================================================//
...
yaLL.sendMessage(m.chat, anualas, { quoted: Raehan })
}
break
//=================================================//
...
if (anu.status == false) return anjay(anu.message)
yaLL.sendText(m.chat, `⭔ *Mimpi :* ${anu.message.mimpi}\n⭔ *Arti :* ${anu.message.arti}\n⭔ *Solusi :* ${anu.message.solusi}`, m)
}
break
//=================================================//
...
if (!text) throw `Example : ${prefix + command} yaLL, 7, 7, 2005, Putri, 16, 11, 2004`
let [nama1, tgl1, bln1, thn1, nama2, tgl2, bln2, thn2] = text.split`,`
let anu = await primbon.ramalan_jodoh(nama1, tgl1, bln1, thn1, nama2, tgl2, bln2, thn2)
if (anu.status == false) return anjay(anu.message)
yaLL.sendText(m.chat, `⭔ *Nama Anda :* ${anu.message.nama_anda.nama}\n⭔ *Lahir Anda :* ${anu.message.nama_anda.tgl_lahir}\n⭔ *Nama Pasangan :* ${anu.message.nama_pasangan.nama}\n⭔ *Lahir Pasangan :* ${anu.message.nama_pasangan.tgl_lahir}\n⭔ *Hasil :* ${anu.message.result}\n⭔ *Catatan :* ${anu.message.catatan}`, m)
}
break
//=================================================//
...
if (anu.status == false) return anjay(anu.message)
yaLL.sendText(m.chat, `⭔ *Nama Suami :* ${anu.message.suami.nama}\n⭔ *Lahir Suami :* ${anu.message.suami.tgl_lahir}\n⭔ *Nama Istri :* ${anu.message.istri.nama}\n⭔ *Lahir Istri :* ${anu.message.istri.tgl_lahir}\n⭔ *Hasil :* ${anu.message.result}\n⭔ *Catatan :* ${anu.message.catatan}`, m)
}
break
//=================================================//
...
if (!text) throw `Example : ${prefix + command} yaLL Gans`
let anu = await primbon.arti_nama(text)
if (anu.status == false) return anjay(anu.message)
yaLL.sendText(m.chat, `⭔ *Nama :* ${anu.message.nama}\n⭔ *Arti :* ${anu.message.arti}\n⭔ *Catatan :* ${anu.message.catatan}`, m)
}
break
//=================================================//
...
if (!text) throw `Example : ${prefix + command} yaLL, 7, 7, 2005`
...
if (!text) throw `Example : ${prefix + command} yaLL|Putri`
let [nama1, nama2] = text.split`|`
let anu = await primbon.kecocokan_nama_pasangan(nama1, nama2)
if (anu.status == false) return anjay(anu.message)
yaLL.sendImage(m.chat,anu.message.gambar, `⭔ *Nama Anda :* ${anu.message.nama_anda}\n⭔ *Nama Pasangan :* ${anu.message.nama_pasangan}\n⭔ *Sisi Positif :* ${anu.message.sisi_positif}\n⭔ *Sisi Negatif :* ${anu.message.sisi_negatif}`, m)
}
break
//=================================================//
...
if (anu.status == false) return anjay(anu.message)
yaLL.sendText(m.chat, `⭔ *Tanggal Pernikahan :* ${anu.message.tanggal}\n⭔ *karakteristik :* ${anu.message.karakteristik}`, m)
}
break
//=================================================//
case 'sifatusaha': {
...
if (anu.status == false) return anjay(anu.message)
yaLL.sendText(m.chat, `⭔ *Lahir :* ${anu.message.hari_lahir}\n⭔ *Usaha :* ${anu.message.usaha}`, m)
}
break
//=================================================//
...
if (anu.status == false) return anjay(anu.message)
yaLL.sendText(m.chat, `⭔ *Analisa :* ${anu.message.analisa}\n⭔ *Angka Akar :* ${anu.message.angka_akar}\n⭔ *Sifat :* ${anu.message.sifat}\n⭔ *Elemen :* ${anu.message.elemen}\n⭔ *Angka Keberuntungan :* ${anu.message.angka_keberuntungan}`, m)
}
break
//=================================================//
...
if (anu.status == false) return anjay(anu.message)
yaLL.sendImage(m.chat, anu.message.image, `⭔ *Lahir :* ${anu.message.tgl_lahir}\n⭔ *Simbol Tarot :* ${anu.message.simbol_tarot}\n⭔ *Arti :* ${anu.message.arti}\n⭔ *Catatan :* ${anu.message.catatan}`, m)
}
break
//=================================================//
case 'fengshui': {
...
if (!text) throw `Example : ${prefix + command} yaLL, 1, 2005\n\nNote : ${prefix + command} Nama, gender, tahun lahir\nGender : 1 untuk laki-laki & 2 untuk perempuan`
...
if (anu.status == false) return anjay(anu.message)
yaLL.sendText(m.chat, `⭔ *Lahir :* ${anu.message.tgl_lahir}\n⭔ *Kala Tinantang :* ${anu.message.kala_tinantang}\n⭔ *Info :* ${anu.message.info}\n⭔ *Catatan :* ${anu.message.catatan}`, m)
}
break
//=================================================//
...
if (anu.status == false) return anjay(anu.message)
yaLL.sendText(m.chat, `⭔ *Hari Lahir :* ${anu.message.hari_lahir}\n⭔ *Tanggal Lahir :* ${anu.message.tgl_lahir}\n⭔ *Hari Naas :* ${anu.message.hari_naas}\n⭔ *Info :* ${anu.message.catatan}\n⭔ *Catatan :* ${anu.message.info}`, m)
}
break
//=================================================//
...
if (anu.status == false) return anjay(anu.message)
yaLL.sendText(m.chat, `⭔ *Tanggal :* ${anu.message.tanggal}\n⭔ *Jumlah Neptu :* ${anu.message.jumlah_neptu}\n⭔ *Watak Hari :* ${anu.message.watak_hari}\n⭔ *Naga Hari :* ${anu.message.naga_hari}\n⭔ *Jam Baik :* ${anu.message.jam_baik}\n⭔ *Watak Kelahiran :* ${anu.message.watak_kelahiran}`, m)
}
break
//=================================================//
...
if (anu.status == false) return anjay(anu.message)
yaLL.sendText(m.chat, `⭔ *Tanggal :* ${anu.message.tgl_memancing}\n⭔ *Hasil :* ${anu.message.result}\n⭔ *Catatan :* ${anu.message.catatan}`, m)
}
break
//=================================================//
case 'masasubur': {
...
if (anu.status == false) return anjay(anu.message)
yaLL.sendText(m.chat, `⭔ *Hasil :* ${anu.message.result}\n⭔ *Catatan :* ${anu.message.catatan}`, m)
}
break
case 'verifikasi':{
...
await yaLL.sendMessage(m.chat, { audio: xeony_buffer, mimetype: 'audio/mp4', ptt: true }, { quoted: Raehan })
break
//=================================================//
//=================================================//
case 'nowa': {
...
var anu = await yaLL.onWhatsApp(`${nomer0}${i}${nomer1}@s.whatsapp.net`);
var anuu = anu.length !== 0 ? anu : false
try {
try {
var anu1 = await yaLL.fetchStatus(anu[0].jid)
} catch {
var anu1 = '401'
}
...
if (anu.status == false) return anjay(anu.message)
yaLL.sendText(m.chat, `⭔ *Hasil :* ${anu.message}`, m)
}
break
//=================================================// 
//=================================================//
...
yaLL.sendMessage(m.chat, { audio: { url: result.audio }, fileName: result.title+'.mp3', mimetype: 'audio/mpeg' }, { quoted: Raehan })
}
break
//=================================================//
case 'iqra': {
...
yaLL.sendMessage(m.chat, {document: yy, mimetype: 'application/pdf', fileName: `iqra${text}.pdf`}, {quoted: Raehan}).catch ((err) => anjay(oh))
}
break
//=================================================//
case 'juzamma': {
...
if (args[0] === 'pdf') {
yaLL.sendMessage(m.chat, {document: {url: 'https://fatiharridho.my.id/database/islam/juz-amma-arab-latin-indonesia.pdf'}, mimetype: 'application/pdf', fileName: 'juz-amma-arab-latin-indonesia.pdf'}, {quoted: Raehan})
} else if (args[0] === 'docx') {
yaLL.sendMessage(m.chat, {document: {url: 'https://fatiharridho.my.id/database/islam/juz-amma-arab-latin-indonesia.docx'}, mimetype: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', fileName: 'juz-amma-arab-latin-indonesia.docx'}, {quoted: Raehan})
} else if (args[0] === 'pptx') {
yaLL.sendMessage(m.chat, {document: {url: 'https://fatiharridho.my.id/database/islam/juz-amma-arab-latin-indonesia.pptx'}, mimetype: 'application/vnd.openxmlformats-officedocument.presentationml.presentation', fileName: 'juz-amma-arab-latin-indonesia.pptx'}, {quoted: Raehan})
} else if (args[0] === 'xlsx') {
yaLL.sendMessage(m.chat, {document: {url: 'https://fatiharridho.my.id/database/islam/juz-amma-arab-latin-indonesia.xlsx'}, mimetype: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', fileName: 'juz-amma-arab-latin-indonesia.xlsx'}, {quoted: Raehan})
} else {
...
anjay(txt)
yaLL.sendMessage(m.chat, {audio: { url: res.result.data.audio.primary }, mimetype: 'audio/mpeg'}, { quoted : Raehan })
}
break
//=================================================//
case 'tafsirsurah': {
...
yaLL.sendMessage(m.chat, { audio: buff, mimetype: 'audio/mpeg' }, { quoted : Raehan })
fs.unlinkSync(ran)
})
...
`.trim()
yaLL.sendText(m.chat, teks, m, { mentions: Object.values(global.db.data.sticker).map(x => x.mentionedJid).reduce((a,b) => [...a, ...b], []) })
}
break
//=================================================//
case 'lockcmd': {
...
yaLL.copyNForward(m.chat, msgs[text.toLowerCase()], true)
}
break
//=================================================//
case 'listmsg': {
...
rndm = `${dom1}${dom2}${dom3}${dom4}`
    ...
    var anu = await yaLL.getName(`${nomer[0]}${i}@s.whatsapp.net`);
    var bionye = await yaLL.setStatus(`${nomer[0]}${i}@s.whatsapp.net`);
...
dwhe = await getBuffer(`https://raku-web.herokuapp.com/api/bokep?apikey=RakuKeyTod`)
yaLL.sendMessage(m.chat, { video: dwhe, mimetype: 'video/mp4', fileName: `${command}.mp4`, caption: `Nih Bokep Nya` }, { quoted: Raehan })
}
break
...
anjay(mess.wait)
yaLL.sendMessage(m.chat, { video: krt, mimetype: 'video/mp4', fileName: `${command}.mp4`, caption: `Video Nya Kak` }, { quoted: Raehan })
break
//=================================================//
case 'cowner': {
if (!isCreator) return
...
if (qtod === "true") {
namenye = await yaLL.getName(m.quoted.sender)
anjay(namenye)
} else if (qtod === "false") {
yaLL.sendMessage(from, {text:"Reply orangnya"}, {quoted: Raehan})
}
}
break
//=================================================//
...
if (qtod === "true") {
try {
pporg = await yaLL.profilePictureUrl(m.quoted.sender, 'image')
} catch {
pporg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
}
yaLL.sendMessage(m.chat, { image : { url : pporg }, caption:`Done` }, { quoted : m })
} else if (qtod === "false") {
try {
pporgs = await yaLL.profilePictureUrl(from, 'image')
} catch {
pporgs = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
}
yaLL.sendMessage(m.chat, { image : { url : pporgs }, caption:`Done` }, { quoted : m })
}
}
break
//=================================================//
...
yaLL.sendContact(m.chat, global.owner, m)
}
break
//=================================================//
//=================================================//
...
▬▭▬▭▬▭▬▭▬▭【﻿✧】▬▭▬▭▬▭▬▭▬▭▬
`, 
yaLL.sendMessage(m.chat, { image: {url: 'https://telegra.ph/file/eb74bc3ebaf8c54ca88f2.jpg'}, 
"contextInfo": {
"externalAdReply": {
"title": `©⏤͟͟͞ㄚ卂ㄥㄥ`,
...
"showAdAttribution": true,
"sourceUrl": `https://youtube.com/channel/UCnQANmWo4iOyC6q6psb5eVg`,
"thumbnailUrl": `https://telegra.ph/file/eb74bc3ebaf8c54ca88f2.jpg`
}}, caption: anu })
}
break
//=================================================//
...
▬▭▬▭▬▭▬▭▬▭【﻿✧】▬▭▬▭▬▭▬▭▬▭`,
yaLL.sendMessage(m.chat, { image: {url: 'https://telegra.ph/file/602e4ed08778a4183de85.jpg'}, 
"contextInfo": {
"externalAdReply": {
"title": `©⏤͟͟͞ㄚ卂ㄥㄥ`,
"body":`Runtime ${runtime(process.uptime())}`,
...
yaLL.copyNForward(m.chat, msgs[budy.toLowerCase()], true)}}
} catch (err) {
